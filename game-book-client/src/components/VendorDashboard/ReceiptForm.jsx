...
const ReceiptForm = ({ businessName = "Bappa Gaming" }) => {
  ...
  const handleWhatsAppShare = async () => {
    ...
    try {
      ...
      // On mobile, try native share API (works great with WhatsApp)
      if (isMobile && navigator.share && navigator.canShare) {
        try {
          const canShareFile = await navigator.canShare({ files: [shareImageData.file] });
          
          if (canShareFile) {
            const shareText = `🎮 *${formData.customerCompany || 'Gaming'}* 🎮\n\n` +
              `📝 नाव / Name: ${formData.customerName}\n` +
              `📅 दिनांक / Date: ${formData.date}\n` +
              `🎯 कंपनी / Company: ${formData.customerCompany || 'N/A'}\n\n` +
              `धन्यवाद! Thank you!`;
            
            await navigator.share({
              title: `Receipt - ${formData.customerName}`,
              text: shareText,
      ...
      // Desktop or fallback: Copy image to clipboard and open WhatsApp
      try {
        // Convert to PNG blob (Clipboard API only supports PNG)
        const img = new Image();
        img.src = shareImageData.dataUrl;
        ...
        toast.success("Image copied to clipboard! Opening WhatsApp...", {
          autoClose: 4000
        });
        
        // Open WhatsApp with bilingual text
        const whatsappMessage = `🎮 *${formData.businessName || 'Gaming'}* 🎮\n\n` +
          `📝 नाव / Name: ${formData.customerName}\n` +
          `📅 दिनांक / Date: ${formData.date}\n` +
          `🎯 कंपनी / Company: ${formData.customerCompany || 'N/A'}\n\n` +
          `📋`;
        ...
        // Try WhatsApp Desktop first, then web
        const whatsappUrls = [
        ...
        // If desktop app doesn't work, open web after a delay
        setTimeout(() => {
        ...
        // Open WhatsApp with bilingual text
        const whatsappMessage = `🎮 *${formData.customerCompany || 'Gaming'}* 🎮\n\n` +
          `📝 नाव / Name: ${formData.customerName}\n` +
          `📅 दिनांक / Date: ${formData.date}\n` +
          `🎯 कंपनी / Company: ${formData.customerCompany || 'N/A'}\n\n` +
          `📎 कृपया डाउनलोड केलेली रसीद संलग्न करा\n` +
          `📎 Please attach the downloaded receipt image\n\n` +
          `धन्यवाद! Thank you!`;
        
        const text = encodeURIComponent(whatsappMessage);
        
        // Try multiple WhatsApp options
